...
const DROP={coinWeight:0.03,potionWeight:0.40,bombWeight:0.40,laserWeight:0.17};

function levelNeed(level){return 80+level*45;}
function applyLevelStats(p){
  p.damage=24+Math.floor((p.level-1)*1.2);
  p.reload=Math.max(.20,.34-(p.level-1)*.007);
  p.speed=230+Math.min(35,(p.level-1)*2);
  p.form=p.level>=15?4:p.level>=10?3:p.level>=5?2:1;
}
function gainXP(p,amount){
  if(!p)return;
  p.xp+=Math.max(1,Math.round(amount));
  while(p.xp>=p.nextXp){p.xp-=p.nextXp;p.level++;p.nextXp=levelNeed(p.level);applyLevelStats(p);}
}
...
function spawnBot(){
  ...
  bots.set(id,{id,name:'BOT-'+id.slice(3),x:pos.x,y:pos.y,angle:rnd(0,Math.PI*2),r:22,hp:50,maxHp:50,speed:rnd(130,150),damage:6,reload:rnd(1.45,1.8),lastShot:0,inv:botInv(),score:0,level:Math.ceil(rnd(1,4)),xp:0,strafe:Math.random()<.5?-1:1,wanderAngle:rnd(0,Math.PI*2),wanderTimer:rnd(1.2,3)});
}
function spawnWild(){
  const id='wild'+wildSeq++,type=Math.random()<.65?'crawler':'orb',hp=type==='crawler'?64:40;
  wilds.set(id,{id,type,x:rnd(70,WORLD.w-70),y:rnd(70,WORLD.h-70),angle:rnd(0,Math.PI*2),r:type==='crawler'?20:15,hp,maxHp:hp,speed:type==='crawler'?rnd(30,50):rnd(55,85),turn:rnd(.6,1.7),inv:{coins:Math.floor(rnd(0,4)),bombs:Math.random()<.70?1:0,potions:Math.random()<.80?1:0,lasers:Math.random()<.22?1:0}});
}
...
function dropFraction(x,y,inv,f=.2){
  ...
  add('laser',Math.floor((inv.lasers||0)*f));
...
function dropWild(x,y,inv){
  ...
  if((inv.lasers||0)>0||Math.random()<.18)spawnPickup('laser',x+rnd(-30,30),y+rnd(-30,30));
}

function respawn(p){
  p.x=WORLD.w/2+rnd(-260,260);p.y=WORLD.h/2+rnd(-210,210);p.hp=100;p.maxHp=100;p.energy=100;p.level=1;p.xp=0;p.nextXp=levelNeed(1);p.score=0;p.laserUntil=0;p.alive=true;p.keys={};p.boost=false;applyLevelStats(p);
}
...
function diePlayer(p,killerId){
  ...
  send(playerSocket(p.id),{t:'dead',stats:{level:p.level,score:p.score,coins:p.coins,bombs:p.bombs,potions:p.potions,lasers:p.lasers}});
}

function send(ws,obj){if(ws.readyState===1)ws.send(JSON.stringify(obj));}
function serialize(){
  return JSON.stringify({t:'state',world:WORLD,players:[...players.values()].filter(p=>p.alive).map(p=>({id:p.id,name:p.name,x:p.x,y:p.y,angle:p.angle,hp:p.hp,maxHp:p.maxHp,energy:p.energy,level:p.level,xp:p.xp,nextXp:p.nextXp,form:p.form,score:p.score,bombs:p.bombs,potions:p.potions,coins:p.coins,lasers:p.lasers,laserActive:p.laserUntil>Date.now()})),bots:[...bots.values()],wilds:[...wilds.values()],bullets,pickups,grenades});
}

wss.on('connection',ws=>{
  const id=crypto.randomBytes(5).toString('hex');
  const p={id,name:'Player-'+id.slice(0,4),x:WORLD.w/2+rnd(-250,250),y:WORLD.h/2+rnd(-200,200),angle:0,r:24,hp:100,maxHp:100,energy:100,level:1,xp:0,nextXp:levelNeed(1),form:1,score:0,damage:24,reload:.34,speed:230,lastShot:0,bombs:2,potions:2,coins:0,lasers:0,laserUntil:0,keys:{},boost:false,alive:true};
  ...
  ws.on('message',raw=>{
    ...
    if(m.t==='shoot'){
      ...
      const spreads=me.form===1?[0]:me.form===2?[-.055,.055]:me.form===3?[-.09,0,.09]:[-.12,-.04,.04,.12];
    ...
    if(m.t==='laser'&&me.lasers>0&&me.laserUntil<=Date.now()){me.lasers--;me.laserUntil=Date.now()+15000;}
  });
...
function tick(){
  ...
  for(const p of players.values()){
    ...
    for(let i=pickups.length-1;i>=0;i--){const q=pickups[i];if(Math.hypot(p.x-q.x,p.y-q.y)<p.r+18){if(q.kind==='coin')p.coins++;else if(q.kind==='bomb')p.bombs++;else if(q.kind==='potion')p.potions++;else if(q.kind==='laser')p.lasers++;pickups.splice(i,1);}}
  }
